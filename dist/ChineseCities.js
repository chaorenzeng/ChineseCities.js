...
(function() {
	...
	window.ChineseCities = function(options) {
		...
		var ChineseCities = {
			...
			getProvince: function() {
				...
				for(i = 0; i < this.provinceArray.length; i++) {
					this.province.options[i + 1] = new Option(this.provinceArray[i], this.provinceArray[i]);
				}
			}
		};
		...
		ChineseCities.province.onchange = function getCity(e) {
			...
			//执行回调返回结果
			hasSelect({'province':e.target.value,'city':ChineseCities.city.value});
		};
		
		ChineseCities.city.onchange = function getRes(e) {
			//切换城市时执行回调返回结果
			hasSelect({'province':ChineseCities.province.value,'city':e.target.value});
		};
